...
def RunFile(file_in):
    ...
    #save file pickle
    out_filename =  file_in[:16] +'Finished_'+ file_in[16:]
    out = open(out_filename,'wb')
    save = {"total_transfer":total_transfer, "extreme_cases": extreme_cases,"n_feasible": n_feasible,"total_time":total_time,"Supplies":all_C, "Requests":all_S, "S_flex":S_flex, "R_flex": R_flex, "carDensity": carDensity, "cpu_t":cpu_t}
...
#csv_out = open('OverallResults-1.csv',"wb")
#out_rows = csv.writer(csv_out)
#out_rows.writerow(["day","Rides","NumCar","NumReq","C:R ratio", "carFlex","ReqFlex", "NumFeasible", "totalCPUtime", "tranferTimes"])
for i in [30]:
    for x in range(1,6):
        S_flex = 1+x/10.0
        for y in range(1,21):
            ...
            file_name = "ChicagoTaxi2014\day%d"%i + tag + '.picke'
            print file_name, "In progress"
            result = RunFile(file_name)
            print file_name, "Finished"
            #save to csv
            #NumCar = len(result['Supplies'])
            #NumReq = len(result['Requests'])
            #out_rows.writerow([i,NumCar+NumReq,NumCar,NumReq,2,S_flex, R_flex,result["n_feasible"], result["total_time"], result["total_transfer"]])
            #print "result updated in csv"
            print
