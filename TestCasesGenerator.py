...
import csv
def GenerateDuration(day_start,day_end):#Generate the supplies and requests for everyday between day_start and day_end
    for i in range(day_start, day_end+1):
        file_name = "ChicagoTaxi2014\day%d.csv" %i
        S_flex = 1.0
        R_flex = 1.0
        for x in range(1,16):
            S_flex =1 + x/10.0
            for y in range(1,21):
                R_flex = 1 +y/10.0
                Generator(file_name,S_flex,R_flex) #file_name, S_flex, R_flex
                print "S_flex", str(S_flex), "R_flex", str(R_flex)
                print
def Generator(file_name,S_flex,R_flex):
    ...
    out_filename = file_name[:-4]+ tag +'.picke'
    ...
    #savefile Txt
    out_filename = file_name[:-4]+tag+'.txt'
